...
void task_init(void) {
  ...
  for(int i = 0; i < LAB_TEST_NUM; ++i) {
    ...
    uint64_t *stack_= alloc_page();
    uint64_t *pgtbl = alloc_page();
    task[i]->sscratch = 0x1001000 + PAGE_SIZE;
    task[i]->satp = (((uint64_t)pgtbl >> 12)&0xFFFFFFFFFFF) | (8<<60)|(i<<44);/*要不要设置mode*/
    create_mapping(pgtbl, task[i]->thread.sp, stack_, 0x1000,
                    (PTE_V|PTE_R|PTE_X|PTE_W));  /*权限是啥*/
    create_mapping(pgtbl, (uint64_t)task[i], task_addr, 0x1000,
                    (PTE_V|PTE_R|PTE_X|PTE_W)); 
