...
void *kmalloc(size_t size) {
  ...
  for (objindex = 0; objindex < NR_PARTIAL; objindex++) {
    if(size <= kmem_cache_objsize[objindex]) {//用kmem_cache_alloc分配内存
      // TODO:
  ...
  if (objindex >= NR_PARTIAL) {
    // TODO:
    p=alloc_pages((size - 1) / PAGE_SIZE + 1);//用alloc_pages分配内存,减一是为了向上取整
...
void kfree(const void *addr) {
  ...
  if (page->flags == PAGE_BUDDY) {
    ...
    clear_page_attr(ADDR_TO_PAGE(addr)->header);//清除页面属性
