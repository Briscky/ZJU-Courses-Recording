...
struct ret_info syscall(uint64_t syscall_num, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5) {
    ...
    case SYS_MMAP: {
        ...
        // return the vm_start
        struct vm_area_struct* vm_area_struct_;
        if((vm_area_struct_=(struct vm_area_struct*)kmalloc(sizeof(struct vm_area_struct)))==NULL){
            ret.a0=-1;
            break;
        }
        vm_area_struct_->vm_start = arg0;
        vm_area_struct_->vm_end = arg0+arg1;
        vm_area_struct_->vm_flags = arg2;
        vm_area_struct_->mapped = 0;
        list_add(&(vm_area_struct_->vm_list),&(current->mm.vm->vm_list));
        ret.a0 = vm_area_struct_->vm_start;
        break;
    ...
    case SYS_MUNMAP: {
        ...
        list_for_each_entry(vma, &current->mm.vm->vm_list, vm_list){
            if(vma->vm_start==arg0&&vma->vm_end==arg0+arg1){
                if(vma->mapped==1){
                    uint64_t pte;
                    pte=get_pte((current->satp&(0xFFFFFFFFFFF))<<12/*取ppn*/,vma->vm_start);
                    free_pages((pte>>10)<<12);
                    
                }
                create_mapping((current->satp&(0xFFFFFFFFFFF))<<12,vma->vm_start,0/*不重要*/,vma->vm_end-vma->vm_start,0/*把valid设为0*/);
