...
void init_buddy_system() {
  ...
  for(int i = 1; i < 8192; ++i) {
    buddy_system.bitmap[i] = get_block_size(i);
  }
...
uint64_t alloc_buddy(int index, unsigned int num) {
  ...
  while(get_size(buddy_system.bitmap[index])>num||check_split(buddy_system.bitmap[index])){
    if(check_split(buddy_system.bitmap[index])){
      if(get_size(buddy_system.bitmap[index*2])>=num){
        index = index*2;
      }
      else{
        index = index*2+1;
  ...
  while(i!=1){
    ...
    buddy_system.bitmap[i/2]=get_size(buddy_system.bitmap[i])>get_size(buddy_system.bitmap[i^1])?get_size(buddy_system.bitmap[i]):get_size(buddy_system.bitmap[i^1]);
    buddy_system.bitmap[i/2]=set_split(get_size(buddy_system.bitmap[i/2]));
    i/=2;
  }
  ...
  if(check_split(buddy_system.bitmap[index])){
    ...
    while(index != 1){
      if(buddy_system.bitmap[index] == buddy_system.bitmap[index^1]){
        ...
        buddy_system.bitmap[index/2] = get_block_size(index/2);
        
      }
      else{
        buddy_system.bitmap[index/2] = set_split((buddy_system.bitmap[index])>get_size(buddy_system.bitmap[index^1])?get_size(buddy_system.bitmap[index]):get_size(buddy_system.bitmap[index^1]));
      }
      index = index/2;
    }
  }
