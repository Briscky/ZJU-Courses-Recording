      ...
      list_for_each_entry(vma, &current->mm.vm->vm_list, vm_list) {
        ...
        if (stval >= vma->vm_start && stval < vma->vm_end) {
          if ( (vma->vm_flags & PTE_V) && (vma->vm_flags & PTE_U) && 
             (((vma->vm_flags & PTE_X) && cause == 12) ||
              ((vma->vm_flags & PTE_R) && cause == 13) ||
              ((vma->vm_flags & PTE_R) && (vma->vm_flags & PTE_W) && cause == 15))
              ) {
              
            uint64_t pa = alloc_pages((vma->vm_end - vma->vm_start) / PAGE_SIZE);
            if (pa == 0) {
              printf("fail to allocate page\n");
              sp_ptr[16] += 4;
              return;
            }
            create_mapping( (current->satp&(0xFFFFFFFFFFF))<<12, vma->vm_start, pa, (vma->vm_end - vma->vm_start), vma->vm_flags);
            vma->mapped = 1;
            return;
          }
          else {
            printf("not permitted to access\n");
            sp_ptr[16] += 4;
            return;
          }
        }
